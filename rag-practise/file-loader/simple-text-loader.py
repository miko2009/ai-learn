...
def load_data_with_llama_index(data_dir):
    simple_file_loader = SimpleDirectoryReader(input_dir=data_dir, required_exts=['.txt'])
...
def load_data_with_langchain_loader(data_dir):
    loader = TextLoader(f"{data_dir}/simple.txt")
    docs = loader.load()
...
def load_data_with_unstructured(data_dir):
    docs = partition_text(f"{data_dir}/simple.txt", strategy="hi_res")
