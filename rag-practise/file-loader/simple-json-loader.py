...
def load_data_with_llama_index(data_dir):
    simple_file_loader = SimpleDirectoryReader(input_dir=data_dir, required_exts=['.json'])
...
def load_data_with_unstructured(data_dir):
    file_name = f"{data_dir}/tes.json"
    print(file_name)
    docs = partition_json(filename=file_name)
