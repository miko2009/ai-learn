...
def load_data_with_llama_index(data_dir):
    simple_file_loader = SimpleDirectoryReader(input_dir=data_dir, required_exts=['.jpg'])
...
def load_data_with_langchain_loader(data_dir):
    loader = UnstructuredImageLoader(f"{data_dir}/wukong-jpg.jpg")
    docs = loader.load()
...
def load_data_with_unstructured(data_dir):
    docs = partition_image(f"{data_dir}/wukong-jpg.jpg", languages=["eng"])
